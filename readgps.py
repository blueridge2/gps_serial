...
import serial
import re
def calculate_checksum(gps_output):
    """
    function: calculate_checksum
    this function calculates the checksum of the gps string and returns 
    true if it is good and false if not.
    """
    checksum = int ( gps_output[-3:],16)
    #print '{:02x}'.format(checksum)
    checksum_string = gps_output[1:-4]
    calculated_checksum = 0
    for i in range(len(checksum_string)):
        calculated_checksum = calculated_checksum ^ ord(checksum_string[i])
...
def main(): 
    ...
    prog = re.compile("^\$GPGGA,([0-9]{1,6}.[0-9]{3}),([0-9]{1,4}.[0-9]{4}),([NS]),([0-9]{1,5}.[0-9]{4}),([EW]),([012]),([0-9]{1,3}),(\d\.\d{1,2}),(\d{1,10}\.\d{1,2}),(M),(-?)(\d{1,10}\.\d{1,2}),(M),,\*([A-F0-9]{2})")
    gps_serial_port = serial.Serial('/dev/ttyAMA0',9600,timeout=1)  # open serial port
    ...
    while(True):
        ...
        if match:
            #print gps_output
            if calculate_checksum(gps_output) is False:
            ...
            if gps_fix == '0':
                print 'No fix detected Continue'
                continue
